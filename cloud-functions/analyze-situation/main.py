...
# --- Model Configuration ---
MODEL_NAME = "gemini-1.5-pro-001" # Or your preferred available model
...
def generate_with_vertex_ai(question: str, history_from_request: list, programs_context: list = None) -> str:
    """
    Generates a response using Vertex AI Gemini, incorporating conversation history
    and a detailed system instruction.

    Args:
        question (str): The user's latest question.
        history_from_request (list): The conversation history from the client.
    ...
    try:
        ...
        # Check if the incoming history is empty (start of conversation)
        # If so, prepend the system instruction and an acknowledgement.
        chat_history_for_model = []
        if not history_from_request:
            logging.info("History is empty, adding system instruction.")
            # Add System Instruction as the first 'user' turn
            chat_history_for_model.append(Part.from_dict({
                "role": "user",
                "parts": [Part.from_text(SYSTEM_INSTRUCTION)]
            }))
            # Add a hypothetical 'model' acknowledgement turn
            chat_history_for_model.append(Part.from_dict({
                "role": "model",
                "parts": [Part.from_text("OK. I understand my role and instructions. How can I help you today?")]
            }))
        else:
             # Convert incoming history (ensure it's in the correct Part format)
             # This assumes the incoming history format matches Vertex AI's requirements
             # {"role": "user"|"model", "parts": [{"text": "..."}]}
             # If frontend sends a different format (like the {'sender':'bot'/'user', 'message':'...'} from the other function),
             # you MUST convert it here. Let's assume it's already correct.
             chat_history_for_model = [Part.from_dict(turn) for turn in history_from_request]


        # Start a chat session using the prepared history
        chat = model.start_chat(history=chat_history_for_model)

        # --- Construct the message to send this turn ---
        # Prepend specific program context IF provided for this turn
        context_prefix = ""
        if programs_context and len(programs_context) > 0:
            context_prefix += "\n\n--- Context on Relevant Programs for this question ---"
            for program in programs_context:
                 ...
                 context_prefix += f"\n- {prog_name}: {prog_desc} (Potential Eligibility Notes: {prog_elig})"
            context_prefix += "\n-----------------------------------------------------\n\n"

        # Combine context (if any) and the latest user question
        full_user_message = f"{context_prefix}{question}"

        logging.info(f"Sending message to model {MODEL_NAME}...")
        # Send the user's message (with prepended context for this turn) to the chat session
        response = chat.send_message(full_user_message)
        logging.info("Received response from Vertex AI chat.")

        # --- Process Response ---
        if response.candidates and response.candidates[0].finish_reason != FinishReason.STOP:
             logging.warning(f"Model response finished with reason: {response.candidates[0].finish_reason}")

        # Return the response text
        # Use response.text which aggregates parts automatically in simple cases
